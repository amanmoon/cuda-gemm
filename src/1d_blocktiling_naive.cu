                                ...
                                T *ptrA, T *ptrB, T *ptrC)
{
    ...
    {
        aCache[threadIdx.y][threadIdx.x] = locA[threadIdx.y * B + blockTileIdx + threadIdx.x];
        bCache[threadIdx.x][threadIdx.y] = locB[(blockTileIdx + threadIdx.x) * C + threadIdx.y];

        __syncthreads();

        for (int cacheIdx = 0; cacheIdx < cK; cacheIdx++)
        {
            accT tmpB = bCache[cacheIdx][threadIdx.y];
            for (int warpIdx = 0; warpIdx < TM; warpIdx++)
            {
                tmpVec[warpIdx] +=
                    aCache[threadIdx.x * TM + warpIdx][cacheIdx] * tmpB;
            }
        }
    ...
    for (int warpIdx = 0; warpIdx < TM; warpIdx++)
    {
        locC[(threadIdx.x * TM + warpIdx) * C + threadIdx.y] =
            tmpVec[warpIdx];
    }
}
