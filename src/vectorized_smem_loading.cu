...
__global__ void vectorizedSmemCaching(const uint A, const uint B, const uint C,
                                      T *__restrict__ ptrA, T *__restrict__ ptrB, T *__restrict__ ptrC)
{
    __shared__ T aCache[cK][cM]; // Transpose
    __shared__ T bCache[cK][cN];

    uint loadRowA = threadIdx.x / (cK / 4); // [0, 128]
    uint loadColA = threadIdx.x % (cK / 4); // [0, 2]

    uint loadRowB = threadIdx.x / (cN / 4); // [0, 128]
    uint loadColB = threadIdx.x % (cN / 4); // [0, 2]
    ...
    for (int blockTileIdx = 0; blockTileIdx < B; blockTileIdx += cK)
    {
        ...
        for (int cacheIdx = 0; cacheIdx < cK; cacheIdx++)
        {
            ...
            for (int regIdx = 0; regIdx < TM; regIdx++)
                regA[regIdx] = aCache[cacheIdx][threadRow * TM + regIdx];
    ...
    for (int col = 0; col < TN; col += 4)
    {
        ...
        {
            float4 tmp;
            tmp.x = tmpMat[row][col + 0];
            tmp.y = tmpMat[row][col + 1];
            tmp.z = tmpMat[row][col + 2];
            tmp.w = tmpMat[row][col + 3];

            reinterpret_cast<float4 *>(&locC[((threadRow * TM) + row) * C + (threadCol * TN) + col])[0] = tmp;
        }
    }
}
...
void benchmark(uint M, uint K, uint N)
{
    ...
    // Warmup
    vectorizedSmemCaching<float, float><<<gridDim, blockDim>>>(M, K, N, d_A, d_B, d_C);
