...
constexpr uint numWrapsPerXDim = 2;
constexpr uint numWrapsPerYDim = 2;

constexpr uint numSubSecInWrapPerXDim = 4;
...
__global__ void wrapTiling(const uint M, const uint K, const uint N, T *__restrict__ ptrA, T *__restrict__ ptrB, T *__restrict__ ptrC)
{
    ...
    for (int blockTileIdx = 0; blockTileIdx < K; blockTileIdx += cacheK)
    {
        ...
        for (int cacheTileIdx = 0; cacheTileIdx < cacheK; cacheTileIdx++)
        {
            ...
            // load register A via float4
            #pragma unroll
            for (int subWrapYIdx = 0; subWrapYIdx < numSubSecInWrapPerYDim; subWrapYIdx++)
            {
                #pragma unroll
                for (int regIdx = 0; regIdx < threadM; regIdx += 4)
                {
                    const int offsetA = (wrapRow * wrapM) + subWrapYIdx * wrapSubM + innerThreadRow * threadM + regIdx;
                    float4 tmpA = reinterpret_cast<const float4 *>(&cacheA[cacheTileIdx][offsetA])[0];

                    registerA[subWrapYIdx][regIdx + 0] = tmpA.x;
                    registerA[subWrapYIdx][regIdx + 1] = tmpA.y;
                    registerA[subWrapYIdx][regIdx + 2] = tmpA.z;
                    registerA[subWrapYIdx][regIdx + 3] = tmpA.w;
                }
            }

            // load register B via float4
            #pragma unroll
            for (int subWrapXIdx = 0; subWrapXIdx < numSubSecInWrapPerXDim; subWrapXIdx++)
            {
                ...
                {
                    const int offsetB = (wrapCol * wrapN) + subWrapXIdx * wrapSubN + innerThreadCol * threadN + regIdx;
                    float4 tmpB = reinterpret_cast<const float4 *>(&cacheB[cacheTileIdx][offsetB])[0];

                    registerB[subWrapXIdx][regIdx + 0] = tmpB.x;
                    registerB[subWrapXIdx][regIdx + 1] = tmpB.y;
                    registerB[subWrapXIdx][regIdx + 2] = tmpB.z;
                    registerB[subWrapXIdx][regIdx + 3] = tmpB.w;
                }
            }
